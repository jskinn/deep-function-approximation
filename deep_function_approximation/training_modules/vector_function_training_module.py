...
class VectorFunctionTrainingModule(LightningModule):
    ...
    def mlflow_logger(self) -> MLFlowLogger | None:
        return next((logger for logger in self.loggers if isinstance(logger, MLFlowLogger)), None)
    ...
    def _log_plots(self) -> None:
        ...
        if mlflow_logger is None:
            return
        x = torch.cat([
            batch_data[0].cpu() for batch_data in self._vis_data
        ], dim=0)
        y = torch.cat([
            batch_data[1].cpu() for batch_data in self._vis_data
        ], dim=0)
        preds = torch.cat([
            batch_data[2].cpu() for batch_data in self._vis_data
        ], dim=0)
        self._vis_data = []
        ...
        for x_idx in range(num_inputs):
            for y_idx in range(num_outputs):
                ...
                axes[x_idx][y_idx].plot(x[:, x_idx], y[:, y_idx], markersize=1, linestyle="none", marker='.')
                axes[x_idx][y_idx].plot(x[:, x_idx], preds[:, y_idx], markersize=1, linestyle="none", marker='.')
                axes[x_idx][y_idx].set_ylim(-1 * y_range, 1 * y_range)
        mlflow_logger.experiment.log_figure(mlflow_logger.run_id, fig, f"true_vs_predicted_epoch{self.current_epoch}.png")
